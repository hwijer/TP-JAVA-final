...
public class EmployeeController {
    ...
    public EmployeeController(EmployeeModel employeeModel, EmployeeView employeeView) {
        ...
        EmployeeController.employeeView.getImport().addActionListener(e -> this.handleImport());
        EmployeeController.employeeView.getExport().addActionListener(e -> this.handleExport());
        this.afficherEmployee();
    }
    ...
    public void handleImport() {
        ...
        if (fileChooser.showOpenDialog(employeeView) == JFileChooser.APPROVE_OPTION) {
            try {
                ...
                employeeView.AfficherFail("Importation réussie !");
            } catch (IOException ex) {
                employeeView.AfficherFail("Une erreur inattendue est survenue : " + ex.getMessage());
            }
        }
    }
    ...
    public void handleExport()  {
        ...
        if (fileChooser.showSaveDialog(employeeView) == JFileChooser.APPROVE_OPTION) {
            try {
                ...
                employeeModel.exportData(filePath, employes);
                employeeView.AfficherFail("Exportation réussie !");
            } catch (Exception ex) {
                employeeView.AfficherFail("Une erreur inattendue est survenue : " + ex.getMessage());
            }
        }
